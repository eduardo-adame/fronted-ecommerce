...
import { Package, Eye, ArrowLeft, X, AlertTriangle, Calendar, DollarSign } from "lucide-react";
...
export function MisCompras({ setVistaActual, user }) {
	...
	useTitle("Mis Compras");

	const fetchData = useCallback(async () => {
		setLoading(true);
		setError("");
		try {
			const data = await apiService.misCompras(user?.id);
	...
	async function openDetail(compra) {
		setDetailTarget(compra);
		setDetailData(null);
		setDetailError("");
		setDetailLoading(true);
		try {
			const data = await apiService.getVenta(compra.id);
			setDetailData(data);
		} catch (err) {
	...
	if (error && !loading && compras.length === 0) {
		return (
	...
	return (
		<div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
			<div className="mb-6 flex items-center gap-3">
				...
				<h1 className="text-xl font-bold text-ink sm:text-2xl">Mis Compras</h1>
			</div>
			...
			{loading ? (
				...
				<EmptyState
					icon={Package}
					title="Aún sin compras"
					message="No has realizado ninguna compra todavía."
				/>
			) : (
						...
						<tbody>
							{compras.map((c) => (
								<tr key={c.id} className="border-b border-border/50 last:border-0 hover:bg-bg/50 transition-colors">
									...
									<td className="px-4 py-3 text-muted">{c.fecha ? formatDate(c.fecha) : "—"}</td>
									<td className="px-4 py-3 text-right font-semibold text-ink">{formatCurrency(c.total)}</td>
									<td className="px-4 py-3 text-right">
										<button
											onClick={() => openDetail(c)}
											className="flex cursor-pointer items-center gap-1 rounded-lg px-2.5 py-1.5 text-xs font-medium text-info transition-colors hover:bg-info-muted focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/20 ml-auto"
										>
			...
			>
				<div className="flex items-center justify-between border-b border-border px-6 py-4">
					<h2 className="text-lg font-bold text-ink">
						Detalle de compra #{detailTarget?.id}
					</h2>
					<button
						onClick={closeDetail}
					...
					{detailLoading ? (
						...
						<div className="space-y-5">
							<div className="grid grid-cols-2 gap-4">
								...
								<div className="col-span-2 flex items-center gap-2.5 rounded-lg bg-surface p-3.5 ring-1 ring-border">
